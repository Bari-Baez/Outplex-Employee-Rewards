...
| `src/app/login/LegacyCanvasLoginPage.tsx` | Búsqueda estática en `src`, `frontend`, `backend`, `tests`, `scripts`, `docs` y `.github` sin un solo consumidor; no es un archivo de ruta, por lo que el App Router nunca lo montaba. | `frontend/modules/access/ui/LandingClient.tsx`, la pantalla de acceso que `/` y `/login` renderizan realmente. | Recuperar desde Git (`chore: initial commit — Outplex v1.0.0`). |

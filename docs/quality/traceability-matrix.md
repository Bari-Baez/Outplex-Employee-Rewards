...
|---|---|---|---|---|---|---|
| Q-01 | Frontera cliente/servidor | `frontend/`, `backend/`, Client Components | `dependency-boundaries.test.mjs` | P0 | Backend | Automatizado |
...
| Q-05 | Calidad estática | `src/`, `frontend/`, `backend/`, `shared/` | typecheck + lint baseline | P1 | Engineering | Automatizado; deuda decreciente |
| F-00 | Política modular de soporte | `backend/modules/support/domain/ticket-policy.ts` | `support-ticket-policy.test.mjs` | P1 | Support Owner/QA | Automatizado |
...
| S-00 | CSRF, redirect y cuerpos HTTP | `src/proxy.ts`, `backend/platform/http/*` | unitarias negativas + E2E cross-site | P0 | Backend/Security | Automatizado |
| S-01 | RLS y service role | SQL Supabase + `createServiceClient` | `database/tests/run-contracts.mjs` | P0 | DBA/Backend | Harness protegido listo; ejecución pendiente |

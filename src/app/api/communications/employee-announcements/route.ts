...
export async function POST(req: Request) {
  try {
    ...
    if (!isStoreLimited) {
      return NextResponse.json({ error: 'Only store owners can create employee announcements.' }, { status: 403 });
    }
    ...
    if (publishAt && status !== 'draft') {
      await enforceDailyEmployeeAnnouncementLimit(publishAt, undefined, auth.profile.id);
    }

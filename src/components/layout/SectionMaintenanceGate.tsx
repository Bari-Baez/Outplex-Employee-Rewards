...
import { resolveToolKeyFromPathname, TOOLS_CATALOG } from '@/lib/tools-catalog';
import { resolveSectionFromLocation, TOOL_SECTIONS_CATALOG } from '@/lib/tool-sections-catalog';
import { useAppAvailability } from '@/components/layout/AppAvailabilityProvider';

export function SectionMaintenanceGate({
  userRole,
  children,
}: {
  userRole: string;
  ...
  const sectionKey = toolKey ? resolveSectionFromLocation(toolKey, pathname, searchParams) : null;
  const enabled = toolKey && sectionKey ? isSectionEnabled(toolKey, sectionKey, { userRole }) : true;

  if (!toolKey || !sectionKey || enabled) {
    return <>{children}</>;
  }

  const toolMeta = TOOLS_CATALOG.find((tool) => tool.key === toolKey);
  const sectionMeta = TOOL_SECTIONS_CATALOG.find((section) => section.toolKey === toolKey && section.sectionKey === sectionKey);

  const fallbackHref = toolKey === 'dashboard' ? '/settings' : toolMeta?.routePrefixes?.[0] ?? '/dashboard';
    ...
    <div className="tool-maint-shell animate-fade-in">
      <div className="tool-maint-card card">
        ...
        <div className="tool-maint-copy">
          ...
          <h1 className="tool-maint-title">
            {sectionMeta?.label ?? 'Esta sección'} está en mantenimiento
          </h1>
